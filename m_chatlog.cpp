...
/*         ServerInstance->Logs->Log("m_chatlog",DEFAULT,"*** %s has joined %s",user->nick.c_str(),chan->name.c_str()); */
         ServerInstance->Logs->Log("m_chatlog_channel",DEFAULT,"*** %s has joined %s",memb->user->nick.c_str(),memb->chan->name.c_str());
    }

    void OnUserPart(Membership* memb, std::string &partmessage, CUList& except_list)
    {
         ServerInstance->Logs->Log("m_chatlog_channel",DEFAULT,"*** %s has left %s: %s",memb->user->nick.c_str(),memb->chan->name.c_str(),partmessage.c_str());
    }


    void OnUserQuit(User* user, const std::string& message, const std::string& oper_message)
    {
         ServerInstance->Logs->Log("m_chatlog_channel",DEFAULT,"*** %s has quit IRC: %s",user->nick.c_str(),message.c_str());
    }

    void OnUserKick(User* source, Membership* memb, const std::string &reason, CUList& excepts)
    {
         ServerInstance->Logs->Log("m_chatlog_channel",DEFAULT,"*** %s has been kicked by %s from %s: %s",memb->user->nick.c_str(),memb->chan->name.c_str(),source->nick.c_str(),reason.c_str());
    }
